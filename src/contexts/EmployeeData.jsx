...
export function EmployeeProvider({ children }) {
    const [loggedIn, setLoggedIn] = useState(JSON.parse(localStorage.getItem('loggedIn')) || null)
    const [employees, setEmployees] = useState(JSON.parse(localStorage.getItem('employees')) || employeesArr)
    const toComplete = (id, task) => {
        let action;
        if (task.status === 'new') {
            action = 'newTask'
        ...
        const newEmployee = { ...employee, tasks: newtasks, tasksNumber: { ...employee.tasksNumber, [action]: employee.tasksNumber[action] - 1, completedTask: employee.tasksNumber.completedTask + 1 } }
    ...
    const toPending = (id, task) => {
        ...
        const newEmployee = { ...employee, tasks: newtasks, tasksNumber: { ...employee.tasksNumber, [action]: employee.tasksNumber[action] - 1, pendingTask: employee.tasksNumber.pendingTask + 1 } }
        const newEmployees = employees.map((employee) => {
            if (employee.name === newEmployee.name) {
                return newEmployee
            } else {
    ...
    const toFailed = (id, task) => {
        ...
        const newEmployee = { ...employee, tasks: newtasks, tasksNumber: { ...employee.tasksNumber, [action]: employee.tasksNumber[action] - 1, failedTask: employee.tasksNumber.failedTask + 1 } }
        const newEmployees = employees.map((employee) => {
            if (employee.name === newEmployee.name) {
                return newEmployee
            } else {
    ...
    return <EmployeeContext.Provider value={[employees, setEmployees, admin, loggedIn, setLoggedIn, toComplete, toPending, toFailed]}>
        {
            children
        }
